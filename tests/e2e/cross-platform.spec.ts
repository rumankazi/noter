...
test.describe('Cross-Platform Window Management', () => {
  ...
  test('should be resizable', async () => {
    ...
    const isResizable = await electronApp.evaluate(
      async ({ BrowserWindow }) => {
  ...
  test('should be maximizable', async () => {
    const isMaximizable = await electronApp.evaluate(
      async ({ BrowserWindow }) => {
        const mainWindow = BrowserWindow.getAllWindows()[0];
  ...
  test('should be minimizable', async () => {
    const isMinimizable = await electronApp.evaluate(
      async ({ BrowserWindow }) => {
        const mainWindow = BrowserWindow.getAllWindows()[0];
  ...
  test('should display platform information correctly', async () => {
    ...
    const platformElement = window.locator(
      '.info-item:has-text("Platform:") span'
    );
  ...
  test('should handle window controls appropriately', async () => {
    const windowControls = await electronApp.evaluate(
      async ({ BrowserWindow }) => {
        const mainWindow = BrowserWindow.getAllWindows()[0];
        return {
  ...
  test('should display real-time information', async () => {
    ...
    const timeElement = window.locator(
      '.info-item:has-text("Current Time:") span'
    );
